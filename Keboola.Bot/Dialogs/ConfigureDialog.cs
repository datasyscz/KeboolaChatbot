...
namespace Keboola.Bot.Dialogs
{
    ...
    {
        [Describe("basic")] [Terms("basic")] basic,
        [Terms("query")] [Describe("query")] query,
        [Terms("login")] [Describe("login")] login,
        [Terms("oauth10")] [Describe("oauth10")] oauth10,
        [Terms("oauth20")] [Describe("oauth20")] oauth20
    }
    ...
    public class ConfigureForm
    {
        ...
        public static IDialog<object> RootConversation()
        {
            return Chain.From(() => FormDialog.FromForm(BuildForm, FormOptions.PromptInStart))
                ...
                .ContinueWith(
                    async (ctx, res) => { return PagingTypeForm.RootConversation(); }
                )
                .ContinueWith<object, object>(
                    async (ctx, res) => { return new EndpointDialog(); });
        }
    }
}